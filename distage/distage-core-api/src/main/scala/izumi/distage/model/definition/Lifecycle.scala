...
object Lifecycle {
  ...
  def fromAutoCloseable[A <: AutoCloseable](acquire: => A): Lifecycle.Basic[Identity, A] = {
    makeSimple(acquire)(_.close)
  }

  def fromAutoCloseableF[F[_], A <: AutoCloseable](acquire: => F[A])(implicit F: DIEffect[F]): Lifecycle.Basic[F, A] = {
    make(acquire)(a => F.maybeSuspend(a.close()))
  }

  def fromExecutorService[A <: ExecutorService](acquire: => A): Lifecycle.Basic[Identity, A] = {
    makeSimple(acquire) {
      es =>
  ...
  /**
    ...
    * }}}
    */
  @open class FromAutoCloseable[+F[_]: DIEffect, +A <: AutoCloseable](acquire: => F[A]) extends Lifecycle.Of(Lifecycle.fromAutoCloseableF(acquire))
